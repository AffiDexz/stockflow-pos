...
export default function OrderDetails() {
  ...
  async function cancel() {
    ...
    try {
      ...
      toast.success(data.refundSimulated ? 'Order cancelled — refund simulated.' : 'Order cancelled, stock restored.');
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <InfoCard label="Payment Status">
          {refund ? <StatusBadge status="REFUNDED" /> : <StatusBadge status={payment?.status} />}
        </InfoCard>
      </div>
      ...
      {refund && (
        <div className="bg-teal-50 border border-teal-200 rounded-xl p-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-teal-600 text-white flex items-center justify-center"><RotateCcw size={20} /></div>
          <div>
            <p className="text-sm font-semibold text-teal-800">{refund.status}</p>
            <p className="text-xs text-teal-600">Refunded {money(refund.amount)} · {formatDate(refund.createdAt)}</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100"><h2 className="font-semibold text-slate-800">Items</h2></div>
      ...
      >
        <p className="text-sm text-slate-600">
          Cancel <strong>{order.reference}</strong>? Stock will be restored
