...
describe('Refund simulation', () => {
  test('cancelling a PAID order refunds and restores stock', async () => {
    const p = await createProduct({ availableStock: 10 });
    const order = await request(app)
      .post('/api/orders')
      .send({ items: [{ productId: p.id, quantity: 3 }] });
    await request(app)
      .post(`/api/orders/${order.body.id}/payment`)
      .send({ outcome: 'success', idempotencyKey: 'refund-key-1' });

    const res = await request(app).post(`/api/orders/${order.body.id}/cancel`);
    expect(res.status).toBe(200);
    expect(res.body.order.status).toBe('REFUNDED');
    expect(res.body.refundSimulated).toBe(true);

    const after = await prisma.product.findUnique({ where: { id: p.id } });
    expect(after.availableStock).toBe(10); // stock restored

    const refunds = await prisma.refund.count({ where: { orderId: order.body.id } });
